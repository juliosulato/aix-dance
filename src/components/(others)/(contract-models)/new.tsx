...
export default function NewContractModelModal({ opened, onClose, mutate }: Props) {
    ...
    async function handleCreateContractModel(data: ContractModelInput) {
        ...
        setIsLoading(true);
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/tenancies/${sessionData.user.tenancyId}/contract-models`, {
                method: "POST",
    ...
    return (
        <Modal
            ...
            centered
        >
            <form onSubmit={handleSubmit(handleCreateContractModel)} className="flex flex-col gap-4">
                <Controller
                    name="title"
                    control={control}
                    render={({ field }) => (
                ...
                <Group justify="flex-end" mt="md">
                    ...
                    <Button type="submit" loading={isLoading}>Salvar Modelo</Button>
                </Group>
            </form>
