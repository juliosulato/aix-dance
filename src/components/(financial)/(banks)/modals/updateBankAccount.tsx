...
import { UpdateBankInput, getCreateBankSchema } from "@/schemas/financial/bank.schema";
...
export default function UpdateBankAccount({ opened, onClose, bankAccount, mutate }: Props) {
    ...
    const { control, handleSubmit, formState: { errors }, register, reset } = useForm<UpdateBankInput>({
    ...
    useEffect(() => {
        if (bankAccount) {
            reset({
                ...
                maintenanceFeeAmount: Number(bankAccount.maintenanceFeeAmount) ?? undefined,
    ...
    async function createBank(data: UpdateBankInput) {
        ...
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/tenancies/${sessionData.user.tenancyId}/banks/${bankAccount?.id}`, {
                method: "PUT",
            ...
            notifications.show({
                message: t("modals.update.notifications.success"),
                color: "green"
            });
