...
function AssignStudents({ opened, onClose, mutate, classData }: Props) {
  ...
  const { data: studentsResponse } = useSWR<StudentComplete[] | PaginatedListResponse<StudentComplete>>(
    () =>
      sessionData?.user.tenantId
        ? `/api/v1/tenants/${sessionData.user.tenantId}/academic/students?limit=500`
        : null,
    fetcher
  );
  ...
  useEffect(() => {
    if (classData && classData.studentClasses) {
      const currentStudentIds = activeEnrollments.map(
        (enrollment: any) => enrollment.student.id
      );
  ...
  async function handleAssignStudents(data: EnrollStudentsInput) {
    ...
    const studentsToArchive = initialStudentIds.filter(
      (id) => !finalStudentIds.includes(id)
    );

    const promises: Promise<Response>[] = [];
    const tenantId = sessionData!.user.tenantId;
    const baseUrl = `/api/v1/tenants/${tenantId}/academic/classes/${classData.id}/enrollments`;
...
credentials: "include",
      ...
      studentsToEnroll.forEach(async (studentId) => {
        const student: StudentComplete = await fetch(
          `/api/v1/tenants/${tenantId}/academic/students/${studentId}`
        ).then((res) => res.json());
        ...
        if (student.subscriptions.length > 1) {
          notifications.show({
            message:
              ...
              " " +
              student.lastName +
              " possui mais de um plano ativo. Verifique manualmente.",
            color: "yellow",
          });
          return;
        }
        ...
        if (!student.classes) {
          notifications.show({
            message:
              "Não foi possível verificar as aulas do aluno " +
              student.firstName +
              " " +
              student.lastName +
              ".",
        ...
        const studentClassesLength = student.classes.filter(
          (c: Class) => c.active == true
        ).length;
        ...
        if (plan.frequency >= studentClassesLength) {
          notifications.show({
            message:
              ...
              " " +
              student.lastName +
              " atingiu o limite de aulas do plano.",
            color: "red",
          });
          return;
        }
        ...
        promises.push(
          fetch(
            `/api/v1/tenants/${tenantId}/academic/students/${studentId}/history`,
            {
              method: "POST",
credentials: "include",
              body: JSON.stringify({
...
credentials: "include",
    ...
    try {
      ...
      notifications.show({
        color: "red",
        message: "Erro ao atribuir estudantes",
      });
    } finally {
      setVisible(false);
    }
  ...
  const handleStudentChange = async (selectedIds: string[]) => {
    ...
    const student = await fetch(
      `/api/v1/tenants/${sessionData?.user.tenantId}/academic/students/${newId}`
    ).then((res) => res.json());
    ...
    else if (!student.classes) error = "não foi possível verificar as aulas.";
    ...
    if (error) {
      notifications.show({
        message: `O aluno ${student.firstName} ${student.lastName} ${error}`,
        color: "red",
      });
      ...
      >
        ...
        >
          ...
          <div className="flex justify-end gap-4 mt-4">
            ...
            <Button type="submit" color="#7439FA" radius="lg" size="md">
              Salvar
            </Button>
          </div>
        </form>
