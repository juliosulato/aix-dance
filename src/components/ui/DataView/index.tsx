...
interface DataViewProps<T> {
  ...
  printable?: boolean;
  onDataChange?: () => void; // Callback when data changes (item added/deleted/updated)
}
...
export default function DataView<T>({
  ...
  const [activeView, setActiveView] = React.useState<"table" | "grade">(
    "grade"
  );
  ...
  // Reset state when data changes (new items added, page changes, etc.)
  React.useEffect(() => {
    // Reset page to 1 when search or filters change
    if (searchValue || Object.values(activeFilters).some((v) => v)) {
      setPage(1);
    }
  ...
  // Function to reset all states (useful for external resets)
  ...
  // Expose reset function to parent component via mutate
  React.useEffect(() => {
    if (mutate && typeof mutate === "function") {
      // Store reset function in a way that parent can access
      (mutate as any)._resetDataView = resetStates;
    }
  }, [mutate, resetStates]);

  // normalize: accept either array or paginated response
  const isServerPaginated = !Array.isArray(data);
  const paginatedResponse = isServerPaginated
    ? (data as PaginatedResponse<T>)
    : null;
  const serverItems = isServerPaginated ? paginatedResponse?.[itemKey] : null;

  const items: T[] = React.useMemo(() => {
    return isServerPaginated
      ? Array.isArray(serverItems)
        ? (serverItems as T[])
        : []
      : (data as T[]);
  }, [isServerPaginated, serverItems, data]);

  // Clean up selectedRows when data changes to avoid invalid IDs
  React.useEffect(() => {
    const currentItemIds = new Set(
      items.map((item) => (item as any).id || (item as any)._id || String(item))
    );
    setSelectedRows((prev) => prev.filter((id) => currentItemIds.has(id)));
  ...
  const processedData = React.useMemo(() => {
    ...
    const activeSelectFilters = Object.entries(activeFilters).filter(
      ([, value]) => value
    );
    if (activeSelectFilters.length > 0) {
      processed = processed.filter((item) =>
        activeSelectFilters.every(
          ([key, value]) => String((item as any)[key]) === value
        )
      );
    }
  ...
  const totalCount = isServerPaginated
    ? paginatedResponse?.pagination?.total ?? items.length
    : processedData.length;
  const serverPage = isServerPaginated
    ? paginatedResponse?.pagination?.page ?? 1
    : activePage; // Use serverPage for pagination control
  const serverLimit = isServerPaginated
    ? paginatedResponse?.pagination?.limit ?? itemsPerPage
    : itemsPerPage; // Use serverLimit for pagination control
  const totalPages = isServerPaginated
    ? paginatedResponse?.pagination?.totalPages ??
      Math.ceil(totalCount / (serverLimit || 1))
  ...
  const onSortForTable: React.Dispatch<
    React.SetStateAction<SortConfig<T> | null>
  > = (v) => {
    const newCfg =
      typeof v === "function"
        ? (v as (prev: SortConfig<T> | null) => SortConfig<T> | null)(
            sortConfig
          )
        : v;
    ...
    <div className="flex flex-col gap-4 md:gap-6">
      ...
      {items.length === 0 && (
        <div className="flex flex-col items-center justify-center h-full text-center bg-white gap-4 rounded-2xl shadow-sm p-3 md:p-6">
          <Image src={notFound} alt="Nada encontrado" style={{ maxWidth: "250px" }} />
          <h2 className="font-semibold text-xl md:text-3xl">
            Ooops... não encontramos nada aqui.
          </h2>
          {openNewModal && (
            <Button
              type="submit"
      ...
      {totalCount > 10 && (
        <Grid
          align="center"
          className="mt-2 md:mt-4 p-4 py-6 md:p-6 lg:py-3 lg:px-6 rounded-3xl bg-white"
        >
          <Grid.Col span={{ base: 12, md: 4 }}>
            <Text size="sm" c="dimmed" ta={{ base: "center", md: "left" }}>
              ...
              –
              {isServerPaginated
                ? Math.min(serverPage * serverLimit, totalCount)
          ...
          <Grid.Col span={{ base: 12, md: 4 }}>
            <Pagination
              ...
              onChange={(p) => {
                if (isServerPaginated && typeof onPageChange === "function") {
                  onPageChange(p, serverLimit, sortConfig);
                } else {
                  setPage(p);
                }
          ...
          <Grid.Col span={{ base: 12, md: 4 }}>
            <Select
              ...
              onChange={(value) => {
                ...
                setPage(1);
                if (isServerPaginated && typeof onPageChange === "function")
                  onPageChange(1, parseInt(newLimit, 10));
              }}
              data={
                activeView === "table"
                  ? ["10", "20", "50", "100"]
