...
export default function VariableSuggestions() {
    ...
    const copyToClipboard = (text: string) => {
        ...
        try {
            ...
            notifications.show({
                message: "Falha ao copiar a variável para a área de transferência.",
                color: 'red',
            });
        }
    ...
    return (
        <div className="p-4 border rounded-lg bg-neutral-50 h-full">
            <h3 className="font-semibold text-gray-800 mb-2">{"Sugestões de Variáveis"}</h3>
            <p className="text-sm text-gray-600 mb-4">
                {"Selecione uma variável para copiá-la para a área de transferência."}
            </p>
            <div className="space-y-4">
                {variableGroups.map(({ groupKey, variables }) => (
                    <div key={groupKey}>
                        <h4 className="font-medium text-sm text-gray-900 mb-2">Variáveis do {groupKey}</h4>
