...
export default function NewStudentContractModal({ opened, onClose, mutate, studentId }: Props) {
    ...
    const { handleSubmit, formState: { errors }, control, reset, watch, setValue } = useForm<CreateStudentContractInput>({
        ...
        defaultValues: { studentId: studentId || '', contractModelId: '', htmlContent: '', manualVariables: {} },
    });
    ...
    const handleClose = () => {
        reset({ studentId: studentId || '', contractModelId: '', htmlContent: '', manualVariables: {} });
        setManualVariables([]);
        onClose();
    };
        ...
        if (studentId && selectedContractModelId && students && contractModels && tenancy) {
            ...
            if (student && model) {
                ...
                Object.entries(presets).forEach(([variable, presetValue]) => {
                    ...
                    switch (presetValue) {
                        case 'STUDENT_FULL_NAME': replacement = `${student.firstName} ${student.lastName}`; break;
                        case 'STUDENT_FIRST_NAME': replacement = student.firstName; break;
                        case 'STUDENT_LAST_NAME': replacement = student.lastName; break;
                        case 'STUDENT_EMAIL': replacement = student.email || ''; break;
                        case 'STUDENT_PHONE': replacement = student.cellPhoneNumber || ''; break;
                        case 'STUDENT_DOCUMENT': replacement = student.documentOfIdentity || ''; break;
                        case 'STUDENT_BIRTH_DATE': replacement = student.dateOfBirth || ''; break;
                        case 'TENANCY_NAME': replacement = tenancy.name; break;
                        case 'TENANCY_DOCUMENT': replacement = tenancy.document; break;
                        case 'TENANCY_EMAIL': replacement = tenancy.email; break;
                        case 'TENANCY_PHONE': replacement = tenancy.phoneNumber; break;
                        case 'CURRENT_DATE': replacement = new Date().toLocaleDateString('pt-BR'); break;
                        default:
                            if (presetValue === 'MANUAL') {
                                foundManuals.push(variable);
                                replacement = manualVariableValues?.[variable] || `{{${variable}}}`; // Mantém a variável se vazia
                            }
                            break;
                    }
                    finalHtml = finalHtml.replace(regex, `<strong>${replacement}</strong>`);
                });
    ...
    }, [studentId, selectedContractModelId, students, contractModels, tenancy, setValue, manualVariableValues]);


    async function handleCreateContract(data: Omit<CreateStudentContractInput, 'htmlContent'>) {
        if (!tenancyId) {
            notifications.show({ color: "red", message: "Sessão inválida. Por favor, faça login novamente." });
            return;
        }
        ...
        try {
            ...
            notifications.show({
                message: "Contrato gerado com sucesso! Agora você pode compartilhar o link para assinatura.",
                color: "green"
            });
    ...
    return (
        <Modal opened={opened} onClose={handleClose} title="Gerar Novo Contrato para Aluno" size="xl" centered>
            <form onSubmit={handleSubmit(handleCreateContract)} className="flex flex-col gap-4">
                ...
                {manualVariables.length > 0 && (
                    <div className='mt-4'>
                        ...
                        {manualVariables.map(variable => (
                            <Controller key={variable} name={`manualVariables.${variable}` as const} control={control} render={({ field }) => (<TextInput label={`Valor para {{${variable}}}`} placeholder={`Digite o valor para ${variable}`} {...field} className="mb-3" required />)} />
                        ))}
                    </div>
                )}
                ...
                <Group justify="flex-end" mt="md">
                    ...
                    <Button type="submit" loading={isLoading}>Gerar Contrato</Button>
                </Group>
            </form>
